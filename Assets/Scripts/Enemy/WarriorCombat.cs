...
public class WarriorCombat : MonoBehaviour
{
    ...
    [Header("Cấu hình Phẫn Nộ (Wrathful)")]
    public float timeUntilWrathful = 10f;
    public float speedMultiplier = 1.5f; 
    public int damageMultiplier = 2; 
    public float rangeMultiplier = 2f;
    [Header("Cấu hình Siêu Phẫn Nộ (Super Wrathful)")]
    public float superSpeedMultiplier = 4f;   // Điên lửa xanh thì chạy nhanh gấp đôi!
    public int superDamageMultiplier = 4;
    public float superRangeMultiplier = 3f;
    ...
    public void HitTarget()
    {
        ...
        if (hitPlayer != null)
        {
            ...
            if (playerController != null)
            {
                ...
                BossWarrior bossWarrior = GetComponent<BossWarrior>();
                if (bossWarrior == null) bossWarrior = GetComponentInParent<BossWarrior>();

                if (bossWarrior != null)
                {
                    bossWarrior.Heal(finalDamage);
                }
                CoolDownWrath();
    ...
    public void EndGuardAndBuff()
    {
        ...
        if (isSuperWrathActive)
        {
            ...
            {
                AudioManager.Instance.PlayVFXWithDuration(AudioManager.Instance.bossMadSound, 1f);
            }

            isBuffApplied = true;
            attackRange = originalRange * superRangeMultiplier;
        }
